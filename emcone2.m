function B=emcone2(R1,DR,I,L,h1,d,N,N1,s,h2)

theta=linspace(0,2*pi*L/d,N);
pos=[(DR.*theta.*(d/2/pi/L)+R1).*cos(theta);(DR.*theta.*(d/2/pi/L)+R1).*sin(theta);theta.*(d/2/pi)];
dtheta=theta(2)-theta(1);
dl=dtheta.*[-(DR.*theta.*(d/2/pi/L)+R1).*sin(theta)+d/2/pi/L.*cos(theta);(DR.*theta.*(d/2/pi/L)+R1).*cos(theta)+d/2/pi/L.*sin(theta);ones(1,N).*(d/2/pi)];
x=linspace(-s,s,N1);
z=linspace(h1,h2,N1);
...
% 可选：整体加一个总标题
sgtitle(['Magnetic field of solenoid (R1=', num2str(R1), 'm, R2=',num2str(R1+DR),'m, L=', num2str(L), 'm)']);
